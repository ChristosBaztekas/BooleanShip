...
public class Human {
	private static List<Human> allHuman = new ArrayList();
	...
	public void instertHuman(Human anyHuman) {
		System.out.println(
		 ...
		 anyHuman = this;//wrong!! is all ready a human
	...
	public static Human createHuman(String class_name, int orgId1) throws IllegalAccessException {
		...
		for(;;) {
			...
			if (pos == -1) {
				...
				System.out.printf("Συνεχίζουμε την διαδικασία με τον άνθρωπο:%s;/n 0 για έξοδο: ", allHuman.get(pos).toString());
				int ans = sc.nextInt();
				if (ans != 0) {
